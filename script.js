...
class ShadowGenerator {
    ...
    initEventListeners() {
        ...
        angleSlider.addEventListener('input', (e) => {
            ...
            document.getElementById('angleValue').textContent = this.lightAngle.toString();
            this.generateShadow();
        });
        elevationSlider.addEventListener('input', (e) => {
            const target = e.target;
            this.lightElevation = parseInt(target.value, 10);
            document.getElementById('elevationValue').textContent = this.lightElevation.toString();
            this.generateShadow();
        });
        intensitySlider.addEventListener('input', (e) => {
            const target = e.target;
            this.shadowIntensity = parseFloat(target.value);
            document.getElementById('intensityValue').textContent = this.shadowIntensity.toFixed(2);
            this.generateShadow();
        });
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (type) {
                case 'foreground':
                    ...
                    document.getElementById('fgStatus').style.display = 'block';
                    break;
                case 'background':
                    this.backgroundImg = img;
                    document.getElementById('bgStatus').style.display = 'block';
                    break;
                case 'depth':
                    this.depthMapImg = img;
                    document.getElementById('depthStatus').style.display = 'block';
                    break;
            }
